...
var AppStores = function AppStores(_ref) {
  ...
  this.dispatch = function (storeName, _ref2) {
    ...
    if (listeners.length > 0) {
      _lodash2.default.merge(preStates, _this.states);
    }
    ...
    if (listeners.length) {
      _lodash2.default.forEach(listeners, function (l) {
        l.handle({ type: type, storeName: storeName, preStates: preStates, states: _this.states });
      });
    }
  };
  ...
  this.setStoreState = function (storeName, state) {
    var storestate = _this.states[storeName];
    if (storestate) {
      var keys = _lodash2.default.keys(state);
  ...
  this.addStore = function (storeName, store) {
    ...
    _this.states[storeName] = {};
    _lodash2.default.merge(_this.states[storeName], state);
    // 初始化actions
    _this.actions[storeName] = {};
    _lodash2.default.merge(_this.actions[storeName], actionFactory({
  ...
  this.delStore = function (storeName) {
    if (_this.states[storeName]) {
      delete _this.states[storeName];
    }
    if (_this.states[storeName]) {
      delete _this.states[storeName];
    }
    if (_this.actions[storeName]) {
      delete _this.actions[storeName];
    }
  };
